<div class="group relative p-4 border-r border-b border-gray-300 sm:p-6">
    <a href="{{ route('product.detail', ['id' => $producto['id']]) }}">
        <div class="rounded-lg overflow-hidden bg-gray-300 aspect-w-1 aspect-h-1 group-hover:opacity-75">
            <img src="{{ file_exists(public_path('assets/products/mains/'.$producto['image'])) ? asset('assets/products/mains/'. $producto['image']) : asset($producto['image']) }}" alt="{{ asset('assets/products/mains/'.$producto['image']) }}" class="w-full h-full object-center object-cover">
        </div>
